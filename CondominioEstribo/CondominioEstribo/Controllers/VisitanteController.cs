...
namespace CondominioEstribo.Controllers
{
    public class VisitanteController : Controller
    {
        ...
        public ActionResult ListarTodosVisitantes()
        {
            return View();
        }
        ...
        [HttpPost]       
        public ActionResult ListarTodosVisitantes(string visitante)
        {
            var q = db.Visitantes.AsQueryable();

            if (!string.IsNullOrEmpty(visitante))
            {
                q = q.Where(x => x.NomeVisitante.Contains(visitante));
            ...
            return RedirectToAction("ListarTodosVisitantes", "Visitante").Mensagem("Visitante não encontrado!");
        ...
        {
           // int idvisitante = Convert.ToInt16(id);
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Visitante visitante = db.Visitantes.Find(id);
            Placa placa = db.Placas.FirstOrDefault(p => p.visitante.VisitanteId .Equals(visitante.VisitanteId));
           
            ViewBag.placa = placa.numeroPlaca.ToString();
            if (visitante == null)
            {
                return HttpNotFound();
            }
